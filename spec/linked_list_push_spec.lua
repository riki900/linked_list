...
describe("linked_list.push(node)", function()
	...
	it("add one item to empty list", function()
		...
		assert.are.equal(1, nodes_length(linked_list.nodes))
		assert.is_true(linked_list.head == node1.address)
		assert.is_true(node1.next == nil)
	end)
	it("add two items to list", function()
		linked_list.push(node1)
		linked_list.push(node2)
		assert.are.equal(2, nodes_length(linked_list.nodes))
		assert.is_true(linked_list.head == node2.address)
		assert.is_true(node2.next == node1.address)
		assert.is_true(node1.next == nil)
	end)
	it("add three items to list", function()
		...
		assert.are.equal(3, nodes_length(linked_list.nodes))
		assert.is_true(linked_list.head == node3.address)
		assert.is_true(node3.next == node2.address)
		assert.is_true(node2.next == node1.address)
		assert.is_true(node1.next == nil)
	end)
end)
